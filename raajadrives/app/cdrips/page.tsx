...
export default async function CdRipListPage() {
  // 👇 1. FILTER: Only fetch albums where type is 'cdrip'
  const { data, error } = await supabase
    .from('releases')
    .select('*')
    .eq('type', 'cdrip') // <--- STRICT FILTER for CD Rips
    .order('created_at', { ascending: false });

  if (error) {
    console.error("Error fetching CD rips:", error);
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white p-6 md:p-12 relative isolate">
      ...
      {/* Background Ambience (Slightly different gradient for distinction) */}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12 text-center md:text-left border-b border-white/10 pb-8">
          <h1 className="text-4xl md:text-5xl font-bold mb-2">CD Rip Archive</h1>
          <p className="text-neutral-400">Uncompressed Red Book Audio (16bit / 44.1kHz)</p>
        </div>
        ...
        <div className="space-y-4">
          {albums.map((album) => (
            <Link 
              key={album.id} 
              // 👇 Pointing to the CD Rips detail page
              href={`/cdrips/${album.slug}`} 
              ...
              <div className="relative flex items-center gap-6 p-4 bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 rounded-xl transition-all duration-300 backdrop-blur-sm">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {/* Quality Badge */}
                    <span className="hidden md:inline-block px-2 py-0.5 text-[10px] font-bold text-black bg-yellow-500 rounded uppercase tracking-wide">
                      {album.quality || '16-BIT'}
                    </span>
                    {/* Format Label */}
                  ...
                  <h2 className="text-xl md:text-2xl font-bold text-white truncate group-hover:text-yellow-400 transition-colors">
                    {album.title}
                  </h2>
